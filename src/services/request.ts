...
function showLoading(): void {
  pendingLoading += 1;
  if (pendingLoading === 1) uni.showLoading({ title: '加载中', mask: true });
}

function hideLoading(): void {
  pendingLoading = Math.max(0, pendingLoading - 1);
  if (pendingLoading === 0) uni.hideLoading();
}
...
export function request<T>(
  ...
  const fail = (err: Error): never => {
    if (!options.silent) uni.showToast({ title: err.message || '网络异常', icon: 'none' });
    throw err;
  };
  ...
  return new Promise<T>((resolve, reject) => {
    uni.request({
      ...
      method,
      data,
      header: { 'x-token': (uni.getStorageSync('wwf_token') as string) || '' },
      success(res) {
        done();
