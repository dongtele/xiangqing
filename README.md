...
当前进度：**顾客端全部完成**，全稿 98 屏做完 **64 屏**
（58 个页面 + 4 个半屏浮层 + 空状态 42 并入 05 + 卡券 39 与 17 合并 / 14 个组件）。
剩余 34 屏全在商家端与入驻流程。
...
| 6d | 商家端接单扩展 + 营销评价 | 21 45 91 92 96 97 / 23 65 66 94 95 47 90 | 待做 |
| 6e | 商家端数据结算 + 店铺团队 | 46 87 89 34 67 88 68 / 50 33 70 71 35 69 72 98 | 待做 |
...
量到的体积：主包 1.4M / 商家分包 284K，两边都留着足够余量；顾客端已全部完成、主包不再增长，
剩余的商家端页面都进商家分包，入驻流程（26 14 27 24 28 29）后续再拆一个 `pages/onboarding` 分包。
...
8. **未做的页面给出明确提示。** 设计稿里指向尚未实现屏号的入口（商家营销与结算、入驻流程）
