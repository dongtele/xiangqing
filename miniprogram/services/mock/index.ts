...
/** 运行期可变副本，模拟服务端状态（接单、上下架、下单等会改数据） */
const state = {
  shop: { ...db.shop },
...
const routes: Record<string, (p: Payload) => unknown> = {
  ...
  'GET /delivery/track': (p): DeliveryTrack | null => {
    ...
    return {
      ...
      activeStage: 1,
      rider: order.rider,
      shopPoint: { left: 70, top: 26 },
      userPoint: { left: 18, top: 64 },
  ...
  'GET /merchant/dashboard': () => ({
    ...
    todos: [
      ...
      { label: '已售罄商品', value: state.merchantGoods.filter((g) => g.stock === 0).length, highlight: false },
    ],
  }),
  ...
  'GET /merchant/goods': (p) => {
    ...
    return { list, categories: ['全部', ...db.categories.filter((c) => c.id !== HOT_CATEGORY_ID).map((c) => c.name)] };
  },
